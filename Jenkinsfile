    ...
    stages {
        ...
        stage('Prepare Tests') {
            steps {
                ...
                '''
                sh 'psw=$(echo -n "{SSHA}"$(echo -n $LDAP_ADMIN_PASSWORD | shasum -a 1 | awk "{print $1}" | base64 -w 0) | base64 -w 0); sed -i -e "s/{WSSO_ADMIN_PASSWORD}/$psw/g" -e "s/{WSSO_ADMIN_USERNAME}/$WSSO_ADMIN_USR/g" wsso.ldif'
